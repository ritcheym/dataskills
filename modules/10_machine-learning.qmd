...
We will review the concept of regression and how we can use regression to make predictions in a new set of data. We will then apply these skills to an open-access psychology dataset. [Download](../_code/prediction.qmd) the `prediction.qmd` file to get started. You will also want to download this [csv file](../files/vivid_df_avg.csv), which contains summary data from the `vivid` dataset.
